...
if($aUserData && isset($aUserData['password']) && is_string($aUserData['password'])){
   $bValidLogin=password_verify($pwdInput,$aUserData['password']);
}

if($bValidLogin){
   unset($_SESSION[$failureKey]); 
   $_SESSION[user_id]=$aU serDat a[id];
   _S ESSION[usern ame]=use rDa ta [usern ame];
SES SION[role]=u serD ata [rol e];
...
$aSessionInfo=isset( _SESSI ON[failureKe y])?$_ SESSION[failureKe y]: [];
