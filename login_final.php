...
// Session hardening on every request - generate CSRF token  
Security::generateCSRFToken(); 

// Rate limiting check BEFORE processing any form data  
$failureKey = 'loginfailed_' . ($_SERVER['REMOTE_ADDR'] ?? '');

// Get recent failures from session or initialize empty array   
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate CSRF token first - reject immediately if invalid    
    $token=$_POST['csrf_token']?? '';       
    if(!Security::validateCSRFToken($token)){
        die('Security validation failed. Please refresh and try again.');
    }

    // Now process login after CSRF passes          
...
// Record this failed attempt for rate limiting purposes                     
